...
public class EducationController : BaseController<Education, int>
{
    //private EducationRepositories _repositories;
    public EducationController(EducationRepositories repositories) : base(repositories)
    {
    ...
    [HttpPost]
    public ActionResult Insert(Education edu)
    {
        try
        {
            var result = _repositories.Insert(edu);
            return result == 0
    ...
    [HttpPut]
    public ActionResult Update(Education edu)
    {
        try
        {
            //if (_repositories.Get(edu.Id) == null)
            //{
            //    return Ok(new { statusCode = 200, message = "Data Didn't Changed or ID Not Found!" });
            //}

            _repositories.Update(edu);
